...
def start_pred(paths, region, cfg, results_path,
	...
	with Dataset(paths['NDVI'], 'r') as ncfile:
		...
		vi_data = ncfile.variables[vi_str][date_idx, lats_idx, lons_idx]
	...
	for lon in lons:
		for lat in lats:
			i += 1
			if i % (len(lats)*len(lons)/200) == 0:
				print str(i/(len(lats)*len(lons)/200.) * 100/200) + '%'
			...
			vi_single = vi_data[:, lat_idx, lon_idx]
			vi_df = pd.DataFrame(vi_single, index=all_dates[date_idx],
								 columns=[vi_str])
